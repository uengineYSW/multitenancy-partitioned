<template>
    <div style="max-height:80vh;">
        ...
        <div class="gs-bundle-of-buttons" style="max-height:10vh;">
            ...
            <v-btn @click="updateSalesOrder" class="contrast-primary-text" small color="primary" :disabled="!hasRole('SalePerson')">
                <v-icon small>mdi-minus-circle-outline</v-icon>UpdateSalesOrder
            </v-btn>
            <v-btn @click="deleteSelectedRows" class="contrast-primary-text" small color="primary" :disabled="!hasRole('SalePerson')">
        ...
        <SalesItemsDetailGrid label="SalesItems" offline v-if="selectedRow" v-model="selectedRow.salesItems"/>
        <v-col>
            <v-dialog
                v-model="openDialog"
...
export default {
    ...
    methods:{
        updateSalesOrder(){
            try{
                this.repository.invoke(this.getSelectedItem(), "updateSalesOrder", null)
                this.$EventBus.$emit('show-success','UpdateSalesOrder 성공적으로 처리되었습니다.')
            }catch(e){
