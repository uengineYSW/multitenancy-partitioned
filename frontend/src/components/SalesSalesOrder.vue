<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-text>
            <String v-if="editMode" label="수주 번호" v-model="value.salesOrderNumber" :editMode="editMode" :inputUI="''"/>
            <String label="수주 담당자" v-model="value.salesPerson" :editMode="editMode" :inputUI="''"/>
        ...
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn  
                :disabled="!userRoles.includes('SalePerson')"
                v-if="!editMode"
                color="primary"
                text
	...
	async created() {
        ...
        methods: {
            ...
            async updateSalesOrder() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['updatesalesorder'].href))
