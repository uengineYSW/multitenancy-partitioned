<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            회사 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            회사
        </v-card-title >        
        ...
        <v-card-text>
            <String v-if="editMode" label="Code" v-model="value.code" :editMode="editMode" :inputUI="''"/>
            <String label="이름" v-model="value.name" :editMode="editMode" :inputUI="''"/>
            <String label="산업" v-model="value.industry" :editMode="editMode" :inputUI="''"/>
            <String label="설립일자" v-model="value.foundedDate" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
	...
	async created() {
        ...
        methods: {
            ...
            async updateCompany(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['updatecompany'].href), params)
